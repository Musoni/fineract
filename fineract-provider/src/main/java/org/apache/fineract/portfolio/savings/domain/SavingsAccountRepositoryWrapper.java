...
public class SavingsAccountRepositoryWrapper {
    ...
    @Transactional(readOnly=true)
    public List<SavingsAccount> findSavingAccountByStatusAndInterest(@Param("status") Integer status) {
        List<SavingsAccount> accounts = this.repository.findSavingAccountByStatusAndInterest(status) ;
        loadLazyCollections(accounts); 
        return accounts ;
    }
    ...
    public boolean doNonClosedSavingAccountsExistForGroup(@Param("groupId") Long groupId) {
        return this.repository.doNonClosedSavingAccountsExistForGroup(groupId) ;
    }
