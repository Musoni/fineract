...
public class GroupsApiResource {
    ...
    public String activateOrGenerateCollectionSheet(@PathParam("groupId") final Long groupId,
        ...
        if (is(commandParam, "activate")) {
            ...
            result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
            return this.toApiJsonSerializer.serialize(result);
        }else if (is(commandParam,"transferGroup")){
        	final CommandWrapper commandRequest = builder.transferGroupBetweenBranches(groupId).build();
        	result = this.commandsSourceWritePlatformService.logCommandSource(commandRequest);
        	return this.toApiJsonSerializer.serialize(result);
        }else if (is(commandParam,"transferStaff")) {
