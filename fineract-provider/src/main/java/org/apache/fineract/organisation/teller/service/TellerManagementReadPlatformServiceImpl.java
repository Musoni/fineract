...
public class TellerManagementReadPlatformServiceImpl implements TellerManagementReadPlatformService {
    ...
    public Collection<CashierTransactionData> fetchTellerTransactionsByTellerId(Long tellerId, Date fromDate, Date toDate) {
            ...
            final String sql = " select * from (select *, @balance:= CASE WHEN txn_type IN (103,101) THEN @balance+ txn_amount ELSE @balance-txn_amount END as balance " +
                    ...
                    "             where renum.enum_value in  ('deposit','withdrawal fee', 'Pay Charge', 'withdrawal', 'Annual Fee', 'Waive Charge', 'Interest Posting', 'Overdraft Interest') )"
                    ...
                    + ctm.loansTxnSchema()
                    +" where  EXISTS ( select renum.enum_value from r_enum_value renum " +
                    "             where renum.enum_value in  ('REPAYMENT_AT_DISBURSEMENT','REPAYMENT', 'RECOVERY_REPAYMENT','DISBURSEMENT', 'CHARGE_PAYMENT', 'WAIVE_CHARGES', 'WAIVE_INTEREST', 'WRITEOFF') )"
                    + " and exists ( select pd.id from m_payment_detail pd left join  m_payment_type pt on pt.id = pd.payment_type_id " +
            ...
            return this.jdbcTemplate.query(sql, ctm, new Object[] { tellerId, null, null, hierarchySearchString,
                    tellerId, null,null,hierarchySearchString,
                    tellerId, null,null, hierarchySearchString });
    }
    ...
    public Page<CashierTransactionData> retrieveCashierTransactions(final Long cashierId, final boolean includeAllTellers,
        ...
        String sql = "select * from  ( select *, @balance:= CASE WHEN txn_type IN (103,101) THEN @balance+ txn_amount ELSE @balance-txn_amount END as balance " +
                " from (select @balance:=0) as p,( select * from ( select " + ctm.cashierTxnSchema()
...
//                hierarchySearchString, cashierId, currencyCode, hierarchySearchString, cashierId, currencyCode, hierarchySearchString });
    ...
    private static final class CashierTransactionSummaryMapper implements RowMapper<CashierTransactionTypeTotalsData> {
        ...
        public String cashierTxnSummarySchema() {
            ...
            sqlBuilder.append(" else ( Date(txn.created_date) between c.start_date AND c.end_date) and  ( TIME(txn.created_date) between TIME(c.start_time) AND TIME(c.end_time))  end) or txn.txn_type = 101) ");
