 ...
 */
@Service
@CommandType(entity = "TELLER", action = "ASSIGNCASHIERTOTELLER")
public class AssignCashierToTellerCommandHandler implements NewCommandSourceHandler {
     ...
     */
    @Autowired
    public AssignCashierToTellerCommandHandler(final TellerWritePlatformService writePlatformService) {
        super();
    ...
    public CommandProcessingResult processCommand(final JsonCommand command) {
        return this.writePlatformService.assignCashierToTeller(command.entityId(), command.subentityId(), command);
    }
}
