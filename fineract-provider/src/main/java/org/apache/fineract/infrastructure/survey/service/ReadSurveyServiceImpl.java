...
public class ReadSurveyServiceImpl implements ReadSurveyService {
    ...
    public List<SurveyDataTableData> retrieveAllSurveys() {
        ...
        while (rs.next()) {
            ...
            surveyDataTables.add(SurveyDataTableData.create(DatatableData.create(appTableName, registeredDatatableName, columnHeaderData,category,null,systemDefined,displayName),
                    enabled));
        }
    ...
    private String retrieveAllSurveySQL(String andClause) {
        ...
        return "select display_name, application_table_name, cf.enabled, registered_table_name,system_defined" + " from x_registered_table "
    ...
    public SurveyDataTableData retrieveSurvey(String surveyName) {
        final String sql = "select display_name, cf.enabled, category, application_table_name, registered_table_name, system_defined" + " from x_registered_table "
                + " left join c_configuration cf on x_registered_table.registered_table_name = cf.name " + " where exists" + " (select 'f'"
                + " from m_appuser_role ur " + " join m_role r on r.id = ur.role_id"
                + " left join m_role_permission rp on rp.role_id = r.id" + " left join m_permission p on p.id = rp.permission_id"
        ...
        while (rs.next()) {
            ...
            datatableData = SurveyDataTableData.create(DatatableData.create(appTableName, registeredDatatableName, columnHeaderData,category,null,systemDefined,displayName),
                    enabled);
    ...
    public GenericResultsetData retrieveSurveyEntry(String surveyName, Long clientId, Long entryId) {
        ...
        return readWriteNonCoreDataService.retrieveDataTableGenericResultSet(surveyName, clientId, null, entryId,false);
