...
public class CodeValuesApiResource {
    ...
    public String retrieveAllCodeValues(@Context final UriInfo uriInfo, @PathParam("codeId") final Long codeId, 
            @QueryParam("includeInactiveCodeValues") final String includeInactiveCodeValues) {

        this.context.authenticatedUser().validateHasReadPermission(this.resourceNameForPermissions);

        Collection<CodeValueData> codeValues = null;
        
        if (StringUtils.equalsIgnoreCase(includeInactiveCodeValues,  "true")) {
            codeValues = this.readPlatformService.retrieveAllCodeValues(codeId);
        }
        
        else {
            codeValues = this.readPlatformService.retrieveAllActiveCodeValues(codeId);
        }
